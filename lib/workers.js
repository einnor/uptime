  ...
  _data.update('checks', newCheckData.id, newCheckData, (err) => {
    if (!err) {
      ...
      if (alertWarranted) {
        ...
        debug({ Info: 'Check outcome not changed, no alert needed' });
      }
    } else {
      debug({ Error: 'Failed to save update to one of the checks '});
    }
  });
};
...
workers.log = (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) => {
  ...
  _logs.append(logFileName, logString, (err) => {
    if (!err) {
      debug({ Success: 'Logged to file succeeded' });
    } else {
      debug({ Error: 'Logginh to file failed' });
    }
  });
};
  ...
  _logs.list(false, (err, logs) => {
    if (!err && logs && logs.length > 0) {
      logs.forEach((logName) => {
        // Compress the datat to a different file
        ...
        _logs.compress(logId, newFileId, (err) => {
          if (!err) {
            ...
            _logs.truncate(logId, (err) => {
              if (!err) {
                debug({ Success: 'Truncated log file' });
              } else {
                debug({ Error: 'Failed to truncate the log file' });
              }
            });
          } else {
            debug({ Error: 'Error compressing one of the log files '});
          }
        });
      });
    } else {
