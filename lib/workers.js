...
workers.log = (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) => {
  ...
  _logs.append(logFileName, logString, (err) => {
    if (!err) {
      console.log({ Success: 'Logged to file succeeded' });
    } else {
      console.log({ Error: 'Logginh to file failed' });
    }
  });
};
          ...
          if (!err) {
            ...
            _logs.truncate(logId, (err) => {
              if (!err) {
                ...
                console.log({ Error: 'Failed to truncate the log file' });
              }
            });
          } else {
            console.log({ Error: 'Error compressing one of the log files '});
          }
        });
      });
    } else {
